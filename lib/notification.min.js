...
/***/ function(module, exports, __webpack_require__) {
	...
	function NotificationComponent(config) {
		...
		userNotifications.then(function (result) {
			...
			var responseData = JSON.parse(result)._embedded.usernotifications;
			var responseIs = [];
			...
			if (_this2.reactComponent) {
				_this2.reactComponent.setState({ notificationList: responseIs });
			} else {
				_this2.notificationList = responseIs;
			}
		}, function (error) {
			console.log(error);
		});
		...
		render: function render() {
			...
			if (!this.state.isDetails) {
				var notificationNodeList = this.props.list.map(function (notification) {
					...
					var jsonObjNotification = JSON.parse(notification);
					return React.createElement(NotificationNode, { detailsClick: _this.showDetails.bind(_this, jsonObjNotification), title: jsonObjNotification.title, icon: jsonObjNotification.icon, key: jsonObjNotification.id, summary: jsonObjNotification.body.substring(0, 30) + "..." });
				});
				return React.createElement(
					"div",

...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports = React.createClass({
		...
		launchCoachmarkFromNewUrl: function launchCoachmarkFromNewUrl() {
			var cmIndex = localStorage.getItem('coachmark.index');
			localStorage.removeItem('coachmark.index');
	
			var cmIds = localStorage.getItem('coachmark.cmIds');
		...
		cmListenerSetup: function cmListenerSetup(cmIds) {
			...
			document.addEventListener('o-cm-backNext-clicked', function (event) {
				var eventIndex = this.getEventIndex(cmIds, event);
				if (eventIndex < 0) {
					return;
				}
		...
		getDisplayCoachmark: function getDisplayCoachmark(cmIds, index) {
			...
			coachmarkData.then(function (result) {
				...
				var isRedirecting = this.redirectIfNewUri(result.uri, cmIds, index);
				if (!isRedirecting) {
					(function () {
		...
		getEventIndex: function getEventIndex(cmIds, event) {
		...
		render: function render() {
			...
			if (!this.state.isDetails) {
				var notificationNodeList = this.props.list.map(function (notification) {
					return React.createElement(NotificationNode, { detailsClick: _this2.showDetails.bind(_this2, notification), title: notification.title, icon: notification.icon, key: notification.id, summary: notification.body.substring(0, 30) + "..." });
				});
				return React.createElement(
					"div",
